 ...
 *
 *   [id0|x0|y0|z0][id1|x1|y1|z1]...[idN-1|xN-1|yN-1|zN-1]
 *    ^-- drone 0 --^^-- drone 1 --^  ^-------- drone N-1 -----^
 *
 ...
 * Le tri se fait sur la coordonnée X : après le tri, les drones
 * sont ordonnés du plus à gauche au plus à droite dans l'espace.
 *
 * POURQUOI TRIER SUR X ?
 * Si la distance minimale est d, alors les deux drones candidats
 * ne peuvent pas être séparés de plus de d sur l'axe X.
 ...
 *
 * *(essaim + i) est STRICTEMENT ÉQUIVALENT à essaim[i] en C,
 * mais nous utilisons uniquement la notation pointeur.
 *
 ...
 *
 * On utilise une FENÊTRE GLISSANTE : pour chaque drone i,
 * on ne compare qu'avec les drones [i+1 ... i+FENETRE].
...
int main(void) {
     ...
     * ----------------------------------------------------------
     * malloc(N * sizeof(Drone)) réserve un bloc CONTIGU de mémoire
     * sur le tas (heap) pour N drones.
     *
     ...
     * Signature : qsort(base, nmemb, size, compar)
     *
     ...
     * ----------------------------------------------------------
     * Le scan linéaire avec fenêtre glissante cherche les deux
     * drones les plus proches en O(n) grâce au pré-tri.
     */
    ...
    /* ----------------------------------------------------------
     * ÉTAPE 5 : Affichage du résultat et déclenchement d'alerte
     * ----------------------------------------------------------
